class P3BALANCEBODYICPVAR {
  static bool iscontrol = true;
  static String ReqNo = '';
  static String TYPE = '';

  static String D01W11 = '';
  static String D01W21 = '';
  static String D01W11_21 = '';
  static String D02W11 = '';
  static String D02W21 = '';
  static String D02W11_21 = '';
  static String D01VOLUME = '';
  static String D02VOLUME = '';
  static String D01NOitem = '';
  static String D02NOitem = '';
  static String WX = '';
  static String DX = '';

  //

  static String Result01 = '';
  static String Result02 = '';

  static String SPEChi = '';
  static String SPEClow = '';

  static String SEND = '';

  static String dip12 = '';
  static String dip13 = '';
  static String dip23 = '';
  //------------

  static String InstrumentName = '';
  //-------------------

  static String Branch = '';
  static String Code = '';
  static String ControlRange = '';
  static String CustFull = '';
  static String CustId = '';
  static String CustShort = '';
  static String DueDate1 = '';
  static String Incharge = '';
  static String ItemName = '';
  static String ItemNo = '';
  static String ItemReportName = '';
  static String ItemStatus = '';
  static String JobType = '';
  static String ListDate = '';
  static String Mag = '';
  static String Position = '';
  static String ProcessReportName = '';
  static String ReceiveDate = '';
  static String RemarkNo = '';
  static String RemarkSend = '';
  static String ReportOrder = '';
  static String ReqDate = '';
  static String ReqUser = '';
  static String RequestRound = '';
  static String RequestSection = '';
  static String RequestStatus = '';
  static String SampleCode = '';
  static String SampleGroup = '';
  static String SampleName = '';
  static String SampleNo = '';
  static String SampleRemark = '';
  static String SampleStatus = '';
  static String SampleTank = '';
  static String SampleType = '';
  static String SamplingDate = '';
  static String SendDate = '';
  static String Std1 = '';
  static String Std2 = '';
  static String Std3 = '';
  static String Std4 = '';
  static String Std5 = '';
  static String Std6 = '';
  static String Std7 = '';
  static String Std8 = '';
  static String Std9 = '';
  static String StdFactor = '';
  static String StdMax = '';
  static String StdMaxL = '';
  static String StdMin = '';
  static String StdMinL = '';
  static String StdSymbol = '';
  static String Temp = '';
  static String UserListAnalysis = '';
  static String UserReceive = '';
  static String UserReject = '';
  static String UserRequestRecheck = '';
  static String UserSend = '';

  static String UID = '';

  static String value = '';

  static bool isShowingMainData = true;

  static String itemName = '';

  static String mem = '-';
}

reserall_P3BALANCEBODYICPVAR() {
  P3BALANCEBODYICPVAR.D01W11 = '';
  P3BALANCEBODYICPVAR.D01W21 = '';
  P3BALANCEBODYICPVAR.D01W11_21 = '';
  P3BALANCEBODYICPVAR.D02W11 = '';
  P3BALANCEBODYICPVAR.D02W21 = '';
  P3BALANCEBODYICPVAR.D02W11_21 = '';
  P3BALANCEBODYICPVAR.D01VOLUME = '';
  P3BALANCEBODYICPVAR.D02VOLUME = '';
  P3BALANCEBODYICPVAR.D01NOitem = '';
  P3BALANCEBODYICPVAR.D02NOitem = '';
}
